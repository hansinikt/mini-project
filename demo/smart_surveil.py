...
def is_climbing(lm, climb_history):
    ...
    Landmarks below MIN_VISIBILITY are treated as not visible,
    which prevents false triggers when legs/ankles are off-screen.
    """
    MIN_VISIBILITY = 0.5   # 0.0–1.0; raise to 0.6 if still too sensitive

    def vis(idx):
        """Return True only if landmark is reliably visible."""
        return hasattr(lm[idx], 'visibility') and lm[idx].visibility >= MIN_VISIBILITY

    # Condition 1: arms raised — only if wrists AND nose are visible
    ...
    # Condition 2: upward motion — only track when hips are visible
    if vis(23) and vis(24):
        ...
        # Hips not visible (person too close / cropped) — clear history,
        # don't count movement to avoid ghost triggers
        climb_history.clear()
        moving_up = False

    # Condition 3: leg spread — only if BOTH ankles are clearly visible
    if vis(27) and vis(28):
        leg_spread = abs(lm[27].x - lm[28].x) > 0.2
    else:
        leg_spread = False   # ankles off-screen → not a valid climbing signal
